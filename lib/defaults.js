...
function load(appName, defaultFolder, ignoreHome) {
	...
	if (!folderArray.some(function(folder) {
		...
		throw Error('Could not find settings file named %s in folders ',
			appNameFilename,
			folderArray.join(','))
	}
...
function getOpts(optsArg, defaultOpts, mustHaves, defaultFile) {
	...
	if (mustHaves) {
		...
		if (!mustHaves.every(function(option) {
			...
			if (!defaultFile) defaultFile = getHomeFolder() + '/haraldops.json'

			msg = 'Required options missing, unable to start.\n' +
				'Options could be stored in a file "' +
				defaultFile +
				+ '" with contents:\n' +
				content
			throw(Error(msg))
