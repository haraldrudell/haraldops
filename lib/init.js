...
function init(optsArg) {
	console.log('haraldops:', haraldops)
	// parse options
	var opts = {}
	...
	if (Array.isArray(optsArg.path)) {
		good = optsArg.path.every(function (path) {
			return haraldops.pushString(opts.paths, path)
		})
	} else {
		if (optsArg.path) good = haraldops.pushString(opts.paths, optsArg.path)
	}
	...
	opts.tmpFolder = haraldops.getTmpFolder()
