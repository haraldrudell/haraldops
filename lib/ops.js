...
function opsconstructor(logger, opts, mockupRequest) {
	...
	if (opts.pingers) {
		...
		if (Array.isArray(list)) list.forEach(pingObject) {
			pinger(pingObject)
		})
	}
	...
	function responder(app, url) {
		app.get(url, function(req, res) {
			if (testmail && // testmail feature is enabled
			 	req.query['mail'] && // it is requested: /status?mail=
			 	req.connection.remoteAddress == '127.0.0.1') { // and from localhost
				// repond with a test mail
				mailsend.sendMail('Test Mail Requested by localhost',
