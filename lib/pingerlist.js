...
function constructor(identifier, maxAge) {
	...
	function checkResponse(title, string, app) {
		...
		do {
			...
			if (!object.hasOwnProperty(app)) {
				result = 'Unmatched app name:' + app
				break
			}

			// check age of the response
			var timestamp = getnum(object, app)
			if (result) break
			var now = unixTimestampNow()
			if (checkStale(now - timestamp, maxAge, 'stale response timestamp')) break

			// examine each pingresult entry
			for (var pingerTitle in object) {
				if (pingerTitle != app) {
					var pingerObj = object[pingerTitle]
					var period = getnum(pingerObj, 'period', pingerTitle)
					if (result) break
					var last = getnum(pingerObj, 'last', pingerTitle)
					if (result) break
					var age = now - last
					if (checkStale(age, 2 * period, pingerTitle + ': stale success time, age s:' + age)) break
				}
			}
		} while (false)
