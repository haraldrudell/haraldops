...
// class variables
var defaultOpts = { logFolder: defaults.getHomeFolder(),
...
var globalStream
var isTeed
var periods = { 'minute': 6, 'hour': 3600, 'day': 86400, 'month': 0, 'year': 0 }
var didWrite
var logFilename
...
function tee(opts) {
	...
	try {
		logFilename = opts.logFolder + '/' + opts.logFile
...
function rotate() {
	if (didWrite && globalStream) {
		// check if file to rotate to already exist
		// file.yymmdd_hhmmss
		...
		var shouldNotExist = fs.statSync(rotatedName)
